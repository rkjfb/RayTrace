...
TEST(Matrix, det2) {
	Matrix2 a(std::array<float, 4>{ 1, 5, -3, 2 });
	EXPECT_EQ(a.det(), 17);
}
...
TEST(Matrix, Submatrix3) {
	...
	Matrix2 expect(std::array<float, 4>{
		-3,2,
		0,6
	});
	EXPECT_EQ(result, expect);
...
TEST(Matrix, Submatrix4) {
	...
	Matrix3 expect(std::array<float, 9>{
		-6, 1, 6,
		-8, 8, 6,
		-7, -1, 1
	});
